...
/**
  ...
  */
abstract class ValidatorN(path: String, repoUser: String, repoName: String, branch: String, labels: List[String], instances: Int, threads: Int, metrics: List[Metric], outputName: String = "fullOutput")
  ...
  override def run(): Unit = {
    ...
    val units = getCurrentUnits(results)


    /* Create instance ids */
    val instanceIds: List[Int] = List.range(0, instances)

    val faultyUnits = instanceIds.par.map(x => runInstance(x, units)).foldLeft(List[String]())((a, b) => a ::: b)
  ...
  /**
    ...
    * @return
    */
  def runInstance(id: Int, currentUnits: List[String]): List[String] = {
    ...
    val result = chunk.foldLeft(List[String]()) {
      (r, x) =>
        ...
        val output = getFaultyUnits(results, currentUnits, x, instanceRepoPath)
  ...
  /**
    ...
    * @return
    */
  def getCurrentUnits(results: List[ResultUnit]): List[String]
  ...
  /**
    ...
    *
    * @param results the list of results
    * @param currentUnits the units in the head version
