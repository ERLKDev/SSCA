...
class Analyser:
	...
	def unRegression(self, df):
		...
		numtypes = self.getNumTypes(df)
		tableData = [["Metric", "Constant", "Coefficient", "P-value", "R\\textasciicircum 2", "Completeness", "Correctness"]]
	...
	def multiRegression(self, df):
		...
		if self.args.kfold:
			...
			for x in range(self.args.kfold):
				...
				paths = df_train['path'].drop_duplicates().values.tolist()


				df_test = df1.copy()
				df_test = df_test[(df_test['path'].isin(paths)) == False]
