...
class ObjectResult(position : RangePosition, val name : String) extends ResultUnit(position){
  ...
  override def toCsv: List[String] = results.foldLeft(List[String]())((a, b) => a ::: b.toCsv.map(x => "|" + name + "$object" + x))

  override def toCsvFunctions: List[String] = {
    results.filter(x => x.isInstanceOf[ResultUnit]).toList.asInstanceOf[List[ResultUnit]]
      .foldLeft(List[String]())((a, b) => a ::: b.toCsvFunctions.map(x => "|" + name + "$object" + x))
  }
  ...
  override def toCsvObject: List[String] = {
    val metrics = results.filter(x => x.isInstanceOf[MetricResult]).toList.asInstanceOf[List[MetricResult]]
      .sortWith((a, b) => a.metricName < b.metricName).foldLeft("|" + name + "$object")((a, b) => a + ", " + b.toCsvObject.mkString)

    val units = results.filter(x => x.isInstanceOf[ResultUnit]).toList.asInstanceOf[List[ResultUnit]]
      .foldLeft(List[String]())((a, b) => a ::: b.toCsvObject.map(x => "|" + name + "$object" + x))
  ...
  override def toCsvObjectSum: List[String] = {
    val metrics = results.filter(x => x.isInstanceOf[MetricResult]).toList.asInstanceOf[List[MetricResult]]
      .sortWith((a, b) => a.metricName < b.metricName).foldLeft("|" + name + "$object")((a, b) => a + ", " + b.toCsvObjectSum.mkString)
    ...
    val units = results.filter(x => x.isInstanceOf[ObjectResult]).toList.asInstanceOf[List[ObjectResult]]
      .foldLeft(List[String]())((a, b) => a ::: b.toCsvObjectSum.map(x => "|" + name + "$object" + x))
  ...
  override def toCsvObjectAvr: List[String] = {
    val metrics = results.filter(x => x.isInstanceOf[MetricResult]).toList.asInstanceOf[List[MetricResult]]
      .sortWith((a, b) => a.metricName < b.metricName).foldLeft("|" + name + "$object")((a, b) => a + ", " + b.toCsvObjectAvr.mkString)
    ...
    val units = results.filter(x => x.isInstanceOf[ObjectResult]).toList.asInstanceOf[List[ObjectResult]]
      .foldLeft(List[String]())((a, b) => a ::: b.toCsvObjectAvr.map(x => "|" + name + "$object" + x))
